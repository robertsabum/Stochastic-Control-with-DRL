...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, hidden_layer_1_dims, hidden_layer_2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        T.nn.init.uniform_(self.hidden_layer_1.weight.data, -hidden_layer_1_weight_range, hidden_layer_1_weight_range)
        T.nn.init.uniform_(self.hidden_layer_1.bias.data, -hidden_layer_1_weight_range, hidden_layer_1_weight_range)
        self.hidden_layer_1_batch_normalization = nn.LayerNorm(self.hidden_layer_1_dims)

        self.hidden_layer_2 = nn.Linear(self.hidden_layer_1_dims, self.hidden_layer_2_dims)
        hidden_layer_2_weight_range = 1./np.sqrt(self.hidden_layer_2.weight.data.size()[0])
        T.nn.init.uniform_(self.hidden_layer_2.weight.data, -hidden_layer_2_weight_range, hidden_layer_2_weight_range)
        T.nn.init.uniform_(self.hidden_layer_2.bias.data, -hidden_layer_2_weight_range, hidden_layer_2_weight_range)
        self.hidden_layer_2_batch_normalization = nn.LayerNorm(self.hidden_layer_2_dims)

        self.action_value = nn.Linear(self.n_actions, self.hidden_layer_2_dims)
        action_value_weight_range = 0.003
    ...
    def forward(self, state, action):
        ...
        state_value = self.hidden_layer_1_batch_normalization(state_value)
        state_value = F.relu(state_value)
        state_value = self.hidden_layer_2(state_value)
        state_value = self.hidden_layer_2_batch_normalization(state_value)
...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, hidden_layer_1_dims, hidden_layer_2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        T.nn.init.uniform_(self.hidden_layer_1.bias.data, -f1, f1)
        self.hidden_layer_1_batch_normalization = nn.LayerNorm(self.hidden_layer_1_dims)
    ...
    def forward(self, state):
        ...
        x = self.hidden_layer_1_batch_normalization(x)
        x = F.relu(x)
        x = self.hidden_layer_2(x)
        x = self.hidden_layer_2_batch_normalization(x)
        x = F.relu(x)
        x = T.tanh(self.mu(x))
...
class PortfolioManager(object):
    def __init__(self, alpha, lr, tau, gamma=0.99, input_dims=35, n_actions=5, max_size=1000000, layer1_size=400, layer2_size=300, batch_size=64):
        self.gamma = gamma
        self.tau = tau
